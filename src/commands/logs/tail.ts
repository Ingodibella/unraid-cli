...
import { applyLogsCommandOptions, defaultLogsCommandDependencies, fetchLogFile, resolveLogsOptions, tailLines, writeRenderedOutput } from './shared.js';
...
export function createLogsTailCommand(
  ...
  return applyLogsCommandOptions(new Command('tail'))
    ...
    .action(async function handleLogsTail(nameOrPath: string) {
      const options = resolveLogsOptions(this);
      const localOptions = this.opts<{ lines: number }>();
      const logFile = await fetchLogFile(nameOrPath, options, dependencies, localOptions.lines);
